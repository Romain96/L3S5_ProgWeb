...
/**
* classe VidéoArchive représente la table VideosArchivees
*/
class VideoArchive extends Model_Base
{
	private $_nom;
	 ...
	 */
	public static function creer_video_archive( array $data )
	{
		$u = new Video( $data );
		$q = self::$_db->prepare('INSERT INTO VideosArchivees( nom, description, duree, premiere_diffusion,
			derniere_diffusion, pays, multi_langue, format_image, nom_emission, numero, vues )
			 values( :nom, :description, :duree, :premiere_diffusion, to_date(:derniere_diffusion, \'DD/MM/YYYY\'),
				 :pays, :multi_langue, :format_image, :nom_emission, :numero, :vues )' );
	...
	public static function get_by_nom( $nom )
	{
		if(is_string( $nom ) )
		{
			$q = self::$_db->prepare( 'SELECT * FROM VideosArchivees WHERE nom LIKE :nom' );
	...
	{
		$p = array();
		$q = self::$_db->prepare( 'SELECT * FROM VideosArchivees ORDER BY nom' );
		$q->execute();
	...
	/**
	* retourne les vidéos archivées de l'émission passée en paramètre
	*/
	public static function get_all_by_nom_emission( $nom_emission )
	{
		$p = array();
		$q = self::$_db->prepare( 'SELECT * FROM VideosArchivees WHERE nom_emission LIKE :nom_emission' );
	...
	public function save()
	{
		if( !is_null( $this->nom ) )
		{
			$q = self::$_db->prepare('UPDATE VideosArchivees SET nom = :nom,
			description = :description, duree = :duree, premiere_diffusion = :premiere_diffusion,
			derniere_diffusion = :derniere_diffusion, pays = :pays, multi_langue = :multi_langue,
			format_image = :format_image, nom_emission = :nom_emission, numero = :numero, vues = :vues');
			$q->bindParam(':nom', $this->get_login(), PDO::PARAM_STR );
			$q->bindParam(':description', $this->get_description(), PDO::PARAM_STR );
			$q->bindParam(':duree', $this->get_duree(), PDO::PARAM_INT );
			$q->bindParam(':premiere_diffusion', $this->get_premiere_diffusion(), PDO::PARAM_STR );
			$q->bindParam(':derniere_diffusion', $this->get_derniere_diffusion(), PDO::PARAM_STR );
			$q->bindParam(':pays', $this->get_pays(), PDO::PARAM_STR );
			$q->bindParam(':multi_langue', $this->get_multi_langue(), PDO::PARAM_INT );
			$q->bindParam(':format_image', $this->get_format_image(), PDO::PARAM_STR );
			$q->bindparam(':nom_emission', $this->get_nom_emission(), PDO::PARAM_STR );
			$q->bindParam(':numero', $this->get_numero(), PDO::PARAM_INT );
			$q->bindParam(':vues', $this->get_vues(), PDO::PARAM_INT );
			$q->execute();
		}
	}
	...
	public function supprimer_video( $nom )
	{
		if( !is_null( $nom ) )
		{
			$q = self::$_db->prepare( 'DELETE FROM VideosArchivees WHERE nom LIKE :nom' );
			$q->bindParam( ':nom', $nom );
			$q->execute();
			$this->_nom = null;
		}
